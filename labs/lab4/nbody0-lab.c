...
double masses[kMaxParticles], positions[kMaxParticles][kNumDims], previousPositions[kMaxParticles][kNumDims],
    previousVelocities[kMaxParticles][kNumDims], forces[kMaxParticles][kNumDims], forceDots[kMaxParticles][kNumDims];

/* other per-particle state variables */
double steps[kMaxParticles], t0[kMaxParticles], t1[kMaxParticles], t2[kMaxParticles], t3[kMaxParticles],
    d1[kMaxParticles][kNumDims], d2[kMaxParticles][kNumDims], d3[kMaxParticles][kNumDims];
...
void readParameters()
{
    ...
    {
        fprintf(stderr,
                "The program currently supports up to %i particles.\n"
...
{
    int i, j, k;
    double deltaVel[kNumDims], deltaPos[kNumDims], deltaForce[kNumDims], deltaForceDot[kNumDims], f1Dot[kNumDims],
        f2Dot[kNumDims], f3Dot[kNumDims];
    double a, b, c, d, e;
...
void outputData()
{
    ...
    double centerOfMass[kNumDims], totalMomentum[kNumDims], totalAngularMomentum[kNumDims];
    ...
    for (i = 0; i < numParticles; i++)
    {
        ...
        for (k = 0; k < kNumDims; k++)
        {
            ...
            positions[i][k] =
                ((((0.05 * d3[i][k] * dt + f2Dot[k] / 12.0) * dt + forceDots[i][k]) * dt + forces[i][k]) * dt +
                 previousVelocities[i][k]) *
                    dt +
                previousPositions[i][k];
            velocity[k] =
                (((0.25 * d3[i][k] * dt + f2Dot[k] / 3.0) * dt + 3.0 * forceDots[i][k]) * dt + 2.0 * forces[i][k]) *
                    dt +
                previousVelocities[i][k];
        }
        ...
        fprintf(outputFile, "%14.7g %14.7g %14.7g %14.7g %14.7g \n", currentTime, masses[i], positions[i][0],
    ...
    printf("\n"
           ...
           "           steps: %14i\n"
           "          energy: %14.7g\n",
           currentTime, numSteps, totalEnergy);
}
...
void advanceParticles()
{
    ...
    double deltaPos[kNumDims], f1Dot[kNumDims], f2Dot[kNumDims], f3Dot[kNumDims], f1[kNumDims];
    ...
    while (currentTime < nextTime)
    {
        ...
        for (j = 0; j < numParticles; j++)
        {
            ...
            for (k = 0; k < kNumDims; k++)
                positions[j][k] =
                    ((forceDots[j][k] * s + forces[j][k]) * s + previousVelocities[j][k]) * s + previousPositions[j][k];
        }
        ...
        {
            f2Dot[k] = d3[i][k] * ((t0[i] - t1[i]) + (t0[i] - t2[i])) + d2[i][k];
            positions[i][k] = (d3[i][k] * 0.05 * dt + f2Dot[k] / 12.0) * (dt * dt * dt * dt) + positions[i][k];
            previousVelocities[i][k] =
                (((d3[i][k] * 0.25 * dt + f2Dot[k] / 3.0) * dt + forceDots[i][k] * 3.0) * dt + forces[i][k] * 2.0) *
                    dt +
                previousVelocities[i][k];
            f1[k] = 0.0;
        }
        ...
        for (k = 0; k < kNumDims; k++)
        {
            ...
            previousPositions[i][k] =
                (((temp4 * dt / 30.0 + f3Dot[k] * 0.05) * dt + f2Dot[k] / 12.0) * dt + f1Dot[k] / 6.0) *
                    (dt * dt * dt) +
                positions[i][k];
            previousVelocities[i][k] =
                (((temp4 * .2 * dt + f3Dot[k] * 0.25) * dt + f2Dot[k] / 3.0) * dt + f1Dot[k] * 0.5) * (dt * dt) +
                previousVelocities[i][k];
        }
...
void writeAllParticleData()
{
    ...
    for (i = 0; i < numParticles; i++)
    {
        fprintf(file,
                ...
                "     d2 = <%f %f %f>\n"
                "     d3 = <%f %f %f>\n\n",
                i, positions[i][0], positions[i][1], positions[i][2], forces[i][0], forces[i][1], forces[i][2],
                forceDots[i][0], forceDots[i][1], forceDots[i][2], previousPositions[i][0], previousPositions[i][1],
                previousPositions[i][2], previousVelocities[i][0], previousVelocities[i][1], previousVelocities[i][2],
                t0[i], t1[i], t2[i], t3[i], steps[i], d1[i][0], d1[i][1], d1[i][2], d2[i][0], d2[i][1], d2[i][2],
